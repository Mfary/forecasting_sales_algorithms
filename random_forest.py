...
class RFRegressor:
    ...
    def random_forest_regression_forecasting(self):
        n_min = 0
        min_error = 100000
        for k in range(2, 150):
                model = RandomForestRegressor(n_estimators=k, max_depth=20)
        ...
        self.rf = RandomForestRegressor(n_estimators=n_min, max_depth=20)
        self.rf.fit(self.x_train, self.y_train)
        rf_pred = self.rf.predict(self.x_test)
        
        rf_pred = rf_pred.reshape(-1,1)
        rf_pred_test_set = np.concatenate([rf_pred,self.x_test], axis=1)
        rf_pred_test_set = self.scaler.inverse_transform(rf_pred_test_set)
        
        result_list = []
        for index in range(0, len(rf_pred_test_set)):
            result_list.append(rf_pred_test_set[index][0] + self.act_sales[index])
        rf_pred_series = pd.Series(result_list,name='rf_pred')
        self.predict_df = self.predict_df.merge(rf_pred_series, left_index=True, right_index=True)
        
        rf_rmse = np.sqrt(mean_squared_error(self.predict_df['rf_pred'], self.monthly_sales['sales'][-12:]))
        rf_mae = mean_absolute_error(self.predict_df['rf_pred'], self.monthly_sales['sales'][-12:])
        rf_r2 = r2_score(self.predict_df['rf_pred'], self.monthly_sales['sales'][-12:])
        ...
        plt.plot(self.predict_df['date'], self.predict_df['rf_pred'])
    ...
    def train(self):
        ...
        self.random_forest_regression_forecasting()
    ...
    def predict(self):
        ...
        for i in range (len(self.monthly_sales_pred.index)):
            ...
            rf_pred = rf_pred.reshape(-1,1)
            rf_pred_test_set = np.concatenate([rf_pred,x], axis=1)
            rf_pred_test_set = self.scaler.inverse_transform(rf_pred_test_set)
...
r = RFRegressor("./train.csv", "./test.csv")
r.train()
r.predict()
print(r.monthly_total)
r.monthly_total.to_csv('result/result_rf.csv')
