...
class KNNRegressor:
    ...
    def knn_regression_forecasting(self):
        k_min = 0
        min_error = 100000
        for k in range(2, 25):
            ...
            if error <= min_error:
                min_error = error
                k_min = k
        print("K minimum is ", k_min)     
        self.knn = KNeighborsRegressor(n_neighbors=k_min)
        self.knn.fit(self.x_train, self.y_train)
        knn_pred = self.knn.predict(self.x_test)
        
        knn_pred = knn_pred.reshape(-1,1)
        knn_pred_test_set = np.concatenate([knn_pred,self.x_test], axis=1)
        knn_pred_test_set = self.scaler.inverse_transform(knn_pred_test_set)
        
        result_list = []
        for index in range(0, len(knn_pred_test_set)):
            result_list.append(knn_pred_test_set[index][0] + self.act_sales[index])
        knn_pred_series = pd.Series(result_list,name='knn_pred')
        self.predict_df = self.predict_df.merge(knn_pred_series, left_index=True, right_index=True)
        
        knn_rmse = np.sqrt(mean_squared_error(self.predict_df['knn_pred'], self.monthly_sales['sales'][-12:]))
        knn_mae = mean_absolute_error(self.predict_df['knn_pred'], self.monthly_sales['sales'][-12:])
        knn_r2 = r2_score(self.predict_df['knn_pred'], self.monthly_sales['sales'][-12:])
        print('K Nearest Neighbor Regression RMSE: ', knn_rmse)
        print('K Nearest Negbor Regression MAE: ', knn_mae)
        print('K Nearest Neighbor Regression R2 Score: ', knn_r2)
        
        
        
        plt.figure(figsize=(15,7))
        plt.plot(self.monthly_sales['date'], self.monthly_sales['sales'])
        plt.plot(self.predict_df['date'], self.predict_df['knn_pred'])
    ...
    def train(self):
        ...
        self.knn_regression_forecasting()
    ...
    def predict(self):
        ...
        for i in range (len(self.monthly_sales_pred.index)):
            ...
            knn_pred = knn_pred.reshape(-1,1)
            knn_pred_test_set = np.concatenate([knn_pred,x], axis=1)
            knn_pred_test_set = self.scaler.inverse_transform(knn_pred_test_set)
            ...
            result_list = []
            for index in range(0, len(knn_pred_test_set)):
                result_list.append(knn_pred_test_set[index][0] + act_sales[index])
            # print(result_list)
...
r.monthly_total.to_csv('result/result_knn.csv')
plt.title("Customer Sales Forecast using Random Forest Regression")
plt.xlabel("Date")
plt.ylabel("Sales")
