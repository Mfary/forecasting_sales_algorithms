...
class BayesianRegressor:
    ...
    def bayesian_regression_forecasting(self):
        self.bayreg = BayesianRidge()
        self.bayreg.fit(self.x_train, self.y_train)
        bayreg_pred = self.bayreg.predict(self.x_test)
        
        bayreg_pred = bayreg_pred.reshape(-1,1)
        bayreg_pred_test_set = np.concatenate([bayreg_pred,self.x_test], axis=1)
        bayreg_pred_test_set = self.scaler.inverse_transform(bayreg_pred_test_set)
        
        result_list = []
        for index in range(0, len(bayreg_pred_test_set)):
            result_list.append(bayreg_pred_test_set[index][0] + self.act_sales[index])
        bayreg_pred_series = pd.Series(result_list,name='bayreg_pred')
        self.predict_df = self.predict_df.merge(bayreg_pred_series, left_index=True, right_index=True)
        
        bayreg_rmse = np.sqrt(mean_squared_error(self.predict_df['bayreg_pred'], self.monthly_sales['sales'][-12:]))
        bayreg_mae = mean_absolute_error(self.predict_df['bayreg_pred'], self.monthly_sales['sales'][-12:])
        bayreg_r2 = r2_score(self.predict_df['bayreg_pred'], self.monthly_sales['sales'][-12:])
        print('Bayesian Regression RMSE: ', bayreg_rmse)
        print('Bayesian Regression MAE: ', bayreg_mae)
        print('Bayesian Regression R2 Score: ', bayreg_r2)
        
        
        
        plt.figure(figsize=(15,7))
        plt.plot(self.monthly_sales['date'], self.monthly_sales['sales'])
        plt.plot(self.predict_df['date'], self.predict_df['bayreg_pred'])
    ...
    def train(self):
        ...
        self.bayesian_regression_forecasting()
    ...
    def predict(self):
        ...
        for i in range (len(self.monthly_sales_pred.index)):
            ...
            bayreg_pred = self.bayreg.predict(x)

            bayreg_pred = bayreg_pred.reshape(-1,1)
            bayreg_pred_test_set = np.concatenate([bayreg_pred,x], axis=1)
            bayreg_pred_test_set = self.scaler.inverse_transform(bayreg_pred_test_set)
            ...
            result_list = []
            for index in range(0, len(bayreg_pred_test_set)):
                result_list.append(bayreg_pred_test_set[index][0] + act_sales[index])
            # print(result_list)
...
r = BayesianRegressor("./train.csv", "./test.csv")
r.train()
r.predict()
